...
func newRowsEvent(table *schema.Table, action string, rows []map[string]interface{}, header *replication.EventHeader) *RowsEvent {
	e := new(RowsEvent)
	...
	tableColCnt := len(table.Columns)
	for _, row := range rows {
		if tableColCnt != len(row) {
			fmt.Println("tableColCnt not match ", tableColCnt, len(row))
			return nil
		}
	}
...
func (r *RowsEvent) handleUnsigned() {
	...
	for i := 0; i < len(r.Rows); i++ {
		...
		for _, columnIdx := range r.Table.UnsignedColumns {
			...
			switch value := r.Rows[i][columnName].(type) {
			case int8:
				r.Rows[i][columnName] = uint8(value)
			case int16:
				r.Rows[i][columnName] = uint16(value)
			case int32:
				...
				if value < 0 && r.Table.Columns[columnIdx].Type == schema.TYPE_MEDIUM_INT {
					r.Rows[i][columnName] = uint32(maxMediumintUnsigned + value + 1)
				} else {
					r.Rows[i][columnName] = uint32(value)
				}
			case int64:
				r.Rows[i][columnName] = uint64(value)
			case int:
				r.Rows[i][columnName] = uint(value)
			default:
				// nothing to do
			}
		}
